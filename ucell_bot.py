...
async def fetch_numbers_from_site():
    ...
    try:
        url = "https://ucell.uz/uz/services/reservation_numbers"
        headers = {"User-Agent": "Mozilla/5.0"}
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                html = await resp.text()

        # Har bir kategoriya uchun raqam qidirish
        cat_map = {
            "simple": "Simple", "steel": "Steel", "gold": "Gold",
            "platinum": "Platinum", "vip": "Vip", "lux": "Lux",
            "luxplus": "Lux+", "special": "Special"
        }
        ...
        # Saytdan ajratib bo'lmasa fallback ishlatamiz
        if len(clean_phones) > 10:
            # Kategoriyalarga ajratish (HTML tartibiga qarab)
            sections = re.split(r'Simple|Steel|Gold|Platinum|Vip|Lux\+|Lux|Special', html)
            keys = list(NUMBERS_DB.keys())
            for i, key in enumerate(keys):
                if i + 1 < len(sections):
                    phones = phone_pattern.findall(sections[i + 1] if i + 1 < len(sections) else "")
                    clean = list(set([re.sub(r'\s+', ' ', p).strip() for p in phones]))
                    if clean:
                        NUMBERS_DB[key]["numbers"] = clean[:30]
                    else:
        ...
        print(f"✅ Raqamlar yangilandi: {last_update_time.strftime('%d.%m.%Y %H:%M')}")

    except Exception as e:
        print(f"❌ Saytdan olishda xato: {e}")
        for key in NUMBERS_DB:
