...
ADMIN_ID = 5546613019  # Abdumalic - buyurtmalar shu yerga keladi
...
async def handle_order_name(chat_id):
    ...
    await send_message(chat_id, "👤 Ismingizni kiriting:")
...
async def handle_order_phone(chat_id):
    ...
    await send_message(chat_id, "📞 Telefon raqamingizni kiriting (bog'lanish uchun):", phone_kb)

async def handle_order_document(chat_id):
    user_states[chat_id]["step"] = "order_document"
    cancel_kb = {"keyboard": [[{"text": "❌ Bekor qilish"}]], "resize_keyboard": True}
    text = (
        "📄 <b>Hujjat rasmini yuboring</b>\n\n"
        "Pasport yoki haydovchilik guvohnomangizning rasmini yuboring\n"
        "<i>(Rasm yoki PDF fayl ko'rinishida)</i>"
    )
    await send_message(chat_id, text, cancel_kb)
...
async def handle_order_complete(chat_id, user_name):
    ...
    # Hujjat rasmini adminga yuborish
    if doc_file_id:
        caption = f"📄 Hujjat - Buyurtma {order_id}\n👤 {order.get('name', '-')}"
...
async def process_update(update):
    ...
    # PDF yoki fayl yuborilgan bo'lsa
    if "document" in msg:
        if state.get("step") == "order_document":
    ...
    if "contact" in msg:
        ...
        if state.get("step") == "order_phone":
            ...
            await handle_order_document(chat_id)
        return
    ...
    if state.get("step") == "order_number":
        ...
        if text == "❌ Bekor qilish":
            ...
            await send_message(chat_id, "❌ Bekor qilindi.", main_menu())
            return
        await send_message(chat_id, "📄 Iltimos, rasm yoki PDF fayl yuboring!")
        return
