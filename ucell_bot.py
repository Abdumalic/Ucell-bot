...
async def send_message_inline(chat_id, text, inline_markup):
    data = {"chat_id": chat_id, "text": text, "parse_mode": "HTML",
...
async def handle_category(chat_id, cat_key, lang="uz"):
    ...
    site_url = "https://ucell.uz/uz/services/reservation_numbers"
    ...
    if lang == "uz":
        ...
        site_btn = "🌐 Saytda to'liq ro'yxatni ko'rish"
        note = "\n\n<i>To'liq ro'yxat uchun saytga o'ting 👇</i>"
        menu_msg = "📝 Raqam yoqdimi? Quyidagi menyudan tanlang:"
    else:
        text = (
            f"━━━━━━━━━━━━━━━━━\n"
        ...
        site_btn = "🌐 Полный список на сайте"
        note = "\n\n<i>Полный список на сайте 👇</i>"
        menu_msg = "📝 Понравился номер? Выберите из меню:"

    if numbers:
        nums_text = "\n".join([f"  ✦ <code>{n}</code>" for n in numbers[:10]])
        text += nums_text + note
    else:
        text += "  — (saytda mavjud)" if lang == "uz" else "  — (доступны на сайте)"

    inline_kb = {"inline_keyboard": [[{"text": site_btn, "url": site_url}]]}
    await send_message_inline(chat_id, text, inline_kb)
    await send_message(chat_id, menu_msg, main_menu(lang))
