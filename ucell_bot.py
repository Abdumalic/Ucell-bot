...
async def fetch_numbers_from_site():
    """
    Ucell saytidan raqamlarni olish.
    Sayt JavaScript bilan yuklanadi, shuning uchun bir nechta usul sinab ko'ramiz:
    1. API endpoint orqali (eng ishonchli)
    ...
    # --- 1-USUL: API ENDPOINT ---
    api_urls = [
        "https://ucell.uz/api/numbers",
        "https://ucell.uz/uz/api/beautiful_numbers",
        "https://ucell.uz/api/beautiful-numbers",
        "https://ucell.uz/uz/services/reservation_numbers/list",
        "https://ucell.uz/api/v1/numbers",
    ]
    ...
    headers = {
        ...
        "Referer": "https://ucell.uz/uz/services/reservation_numbers",
        "X-Requested-With": "XMLHttpRequest",
    }

    phone_pattern = re.compile(r'\+998[\s\-]?\d{2}[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}')

    # Kategoriya kalitlari
    cat_keywords = {
        "simple":   ["Simple", "simple", "Oddiy"],
        "steel":    ["Steel", "steel"],
        "gold":     ["Gold", "gold", "Oltin"],
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # API endpointlarni sinab ko'rish
                ...
                async with session.get(api_url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status == 200:
                        ...
                        if "json" in content_type:
                            ...
                            print(f"✅ API topildi: {api_url}")
                            parsed = parse_api_response(data)
                            if parsed:
                                last_update_time = datetime.now()
                                print(f"✅ API dan raqamlar olindi!")
                                return
            except Exception as e:
                print(f"⚠️ API {api_url}: {e}")
                continue

        # --- 2-USUL: HTML PARSING ---
        html_urls = [
            "https://ucell.uz/uz/services/reservation_numbers",
            "https://ucell.uz/uz/beautiful_numbers",
            "https://ucell.uz/ru/services/reservation_numbers",
            "https://ucell.uz/en/beautiful_numbers",
        ]
        ...
        for page_url in html_urls:
            try:
                ...
                all_phones = phone_pattern.findall(html)
                clean_phones = list(set([re.sub(r'[\s\-]+', ' ', p).strip() for p in all_phones]))

                print(f"📄 {page_url}: {len(clean_phones)} ta raqam topildi")

                if len(clean_phones) >= 5:
                    # Kategoriyalarga ajratish
                    # HTML bo'limlarni ajratish (kategoriya nomlari orasidagi matn)
                    found_any = False
                    for key, keywords in cat_keywords.items():
                        # Har bir kategoriya uchun HTML bo'limini izlash
                        for kw in keywords:
                            # Kategoriya nomidan keyingi qism
                            pattern = re.compile(
                                rf'{re.escape(kw)}.*?(?=Simple|Steel|Gold|Platinum|VIP|Vip|Lux\+|Lux|Special|$)',
                                re.DOTALL | re.IGNORECASE
                            )
                            match = pattern.search(html)
                            if match:
                                section_phones = phone_pattern.findall(match.group())
                                clean_section = list(set([
                                    re.sub(r'[\s\-]+', ' ', p).strip()
                                    for p in section_phones
                                ]))
                                if clean_section:
                                    NUMBERS_DB[key]["numbers"] = clean_section[:50]
                                    found_any = True
                                    print(f"  ✦ {key}: {len(clean_section)} ta raqam")
                                    break

                    if not found_any:
                        # Barcha raqamlarni taqsimlash (kategoriyasiz)
                        chunk_size = max(1, len(clean_phones) // len(NUMBERS_DB))
                        keys = list(NUMBERS_DB.keys())
                        for i, key in enumerate(keys):
                            start = i * chunk_size
                            NUMBERS_DB[key]["numbers"] = clean_phones[start:start + chunk_size]

                    last_update_time = datetime.now()
                    print(f"✅ HTML dan raqamlar olindi: {page_url}")
                    return

            except Exception as e:
                print(f"⚠️ HTML {page_url}: {e}")
                continue

    # --- 3-USUL: FALLBACK ---
    print("⚠️ Saytdan olib bo'lmadi — zaxira raqamlar ishlatilmoqda")
    for key in NUMBERS_DB:
        if not NUMBERS_DB[key]["numbers"]:
            NUMBERS_DB[key]["numbers"] = FALLBACK_NUMBERS.get(key, [])

    last_update_time = datetime.now()


def parse_api_response(data):
    """JSON API javobidan raqamlarni ajratish"""
    phone_pattern = re.compile(r'\+998[\s\-]?\d{2}[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}')
    try:
        # Agar list bo'lsa
        if isinstance(data, list):
            for item in data:
                if isinstance(item, dict):
                    cat = str(item.get("category", item.get("type", "simple"))).lower()
                    number = item.get("number", item.get("phone", ""))
                    for key in NUMBERS_DB:
                        if key in cat or cat in key:
                            if phone_pattern.match(str(number)):
                                NUMBERS_DB[key]["numbers"].append(number)
            return any(NUMBERS_DB[k]["numbers"] for k in NUMBERS_DB)
            ...
            for key in NUMBERS_DB:
                if key in data:
                    ...
                    if isinstance(nums, list):
                        NUMBERS_DB[key]["numbers"] = [str(n) for n in nums][:50]
            return any(NUMBERS_DB[k]["numbers"] for k in NUMBERS_DB)

    except Exception as e:
        print(f"API parse xato: {e}")
    return False
...
async def process_update(update):
    ...
    cat_map = {
        "Simple": "simple", "Steel": "steel", "Gold": "gold",
        "Platinum": "platinum", "VIP": "vip", "Lux+": "luxplus",
        "Lux": "lux", "Special": "special"
    }
    for key, val in cat_map.items():
        if key in text:
            await handle_category(chat_id, val, lang)
            return
